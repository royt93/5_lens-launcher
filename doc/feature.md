...
1. **FISH-008 — Multi-lens workspaces (Phase 1)**: Hỗ trợ nhiều không gian lens độc lập (Room DB migration & data model).
2. **BUILD-001 — Reproducible dependencies & packaging**: Pin dependency verification, tối ưu ProGuard keep rules, loại bỏ native collisions (.so pickFirst).
